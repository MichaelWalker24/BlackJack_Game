...
def clear():
    ...
    command = 'clear'
    # If statement to check to see what OS the computer running the file is operating with. The "clear" command in Windows is "cls", so if the computer is running Windows it changes the command so it works
...
def deal_card():
    ...
    # Create the cards list for each card, Ace through King 
...
# Function to score of cards that are pulled from the deck
def calculate_score(cards): # Note this "cards" argument is separate from the cards variable in the above function due to Scope
    # Create the docstring
    ...
    if sum(cards) == 21 and len(cards) == 2: # sum() and len() are both built in Python Functions
        # return 0 to represent a BlackJack hand, we will use this instead of 21 to differentiate it from a normal score of 21
        return 0
    # Create am of statement that says if 11 is in the cards list, and the sum is less than 21, change 11 to 1 so as to not bust
...
def compare(user_score, computer_score): # Pass in both the user's score and the computer's as arguments
...
def playgame():
    ...
    while not is_game_over: # We use the "not" keyword to indicate state this loop will continue to run while is_game_over=False
